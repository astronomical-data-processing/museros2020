...
class MuserFrame(MuserBase):
    ...
    def set_array(self, sub_array=1):
        ...
        (
            self.sub_channels,
            self.antennas,
            self.dr_output_antennas,
            self.frame_number,
            self.polarization_number,
        ) = muser_para[self.sub_array - 1]

        log.debug(
            "Array: muser-%d: antennas:%d, frame: %d, polarization:%d"
            % (
                self.sub_array,
                self.antennas,
                self.frame_number,
    ...
    def init_data_buffer(self):
        self.block_data = numpy.zeros(
            (self.antennas, self.antennas, self.sub_channels), dtype=complex
        )
        if self.is_loop_mode:
            self.block_full_data = numpy.zeros(
                (
                    self.antennas,
                    self.antennas,
                    self.sub_channels * self.frame_number,
                    self.polarization_number,
                ),
                dtype=complex,
            )
            ...
            self.block_full_data = numpy.zeros(
                (self.antennas, self.antennas, self.sub_channels, 1), dtype=complex
            )
    ...
    def search_frame_header(self):
        ...
        if pos > 0:
            if pos % offset[self.sub_array - 1] != 0:
                self.in_file.seek(
                    ((pos // offset[self.sub_array - 1]) + 1)
                    * offset[self.sub_array - 1]
                )
    ...
    def seek_frame_header(self):
        ...
        if pos > 0:
            self.in_file.seek(
                (pos // offset[self.sub_array - 1]) * offset[self.sub_array - 1]
            )
        self.in_file.seek(32, 1)
        return True
    ...
    def search_next_full_frame_head(self):
        while True:
            ...
            if self.is_loop_mode == False:
                ...
                if (
                    self.is_loop_mode and self.sub_band == 0 and self.polarization == 0
                ):  # Find file in previous 1 minute
                    break
        self.skip_frames(0)
    ...
    def read_one_frame(self, search=True):
        ...
        if self.current_frame_time < Time("2015-05-01T00:00:00", format="isot"):
            self.version = False
        else:
            self.version = True

        # read out parameters
        self.current_frame_header.frequency_switch = struct.unpack(
            "I", self.in_file.read(4)
        )[0]
        self.in_file.seek(128, 1)
        self.current_frame_header.bandwidth = struct.unpack("I", self.in_file.read(4))[
            0
        ]
        self.current_frame_header.quantization_level = struct.unpack(
            "I", self.in_file.read(4)
        )[0]
        self.current_frame_header.delay_switch = struct.unpack(
            "I", self.in_file.read(4)
        )[0]
        self.current_frame_header.strip_switch = struct.unpack(
            "I", self.in_file.read(4)
        )[0]
        self.current_frame_header.sub_band_switch = struct.unpack(
            "I", self.in_file.read(4)
        )[0]
        ...
        if self.version == True:
            if (
                self.current_frame_header.sub_band_switch & 0xFFFF
            ) == 0x3333:  # Polarization
                self.polarization = 0
            else:
                self.polarization = 1
        ...
        self.sub_band_switch = (
            self.current_frame_header.sub_band_switch >> 16
        )  # Sub_band frequency
        ...
        if self.sub_array == 1:
            ...
            if self.version == False:
                if self.sub_band_switch in [0x3333, 0x7777, 0xBBBB, 0xCCCC]:
                    ...
                    self.numrows = 1
                    self.groups = (
                        60 * 1000 / 25 * 8 * (self.antennas * (self.antennas - 1) // 2)
                    )
                    ...
                    self.groups = (
                        ...
                        * 1000
                        / 25
                        * 8
                        * (self.antennas * (self.antennas - 1) // 2)
                        / 2
                    )
                ...
                if self.sub_band_switch in NON_LOOP_MODE_LOW:
                    ...
                    self.current_frame_header.sub_band_switch = struct.unpack(
                        "B", self.in_file.read(1)
                    )[0]
                    # print "self.current_frame_header.sub_band_switch", self.current_frame_header.sub_band_switch
                    self.polarization = LOOP_MODE_LOW[
                        self.current_frame_header.sub_band_switch
                    ][0]
                    self.channel_group = LOOP_MODE_LOW[
                        self.current_frame_header.sub_band_switch
                    ][1]
                    self.frequency = LOOP_MODE_LOW[
                        self.current_frame_header.sub_band_switch
                    ][2]
                    self.freqid = LOOP_MODE_LOW[
                        self.current_frame_header.sub_band_switch
                    ][3]
            ...
            if self.sub_band_switch in NON_LOOP_MODE_HIGH:  # Sub_band frequency
                ...
                self.in_file.seek(4, 1)
                self.current_frame_header.sub_band_switch = struct.unpack(
                    "H", self.in_file.read(2)
                )[0]
                self.polarization = LOOP_MODE_HIGH[
                    self.current_frame_header.sub_band_switch
                ][0]
                self.channel_group = LOOP_MODE_HIGH[
                    self.current_frame_header.sub_band_switch
                ][1]
                self.frequency = LOOP_MODE_HIGH[
                    self.current_frame_header.sub_band_switch
                ][2]
                self.freqid = 1
            ...
            for channel in range(0, self.sub_channels, 2):
                ...
                for antenna in range(0, self.dr_output_antennas, 4):
                    ...
                    if channel == self.sub_channels - 2:
                        ...
                        (
                            self.par_delay[antenna],
                            self.par_delay[antenna + 1],
                            self.par_delay[antenna + 2],
                            self.par_delay[antenna + 3],
                        ) = self.convert_time_offset(buff)[:]
            ...
            for channel in range(
                ...
                for antenna in range(0, 8):
                    ...
                    if channel == 14 or channel == 15:
                        ...
                        buff = self.in_file.read(16)
                        (
                            self.par_delay[0 + antenna * 8 + newch * 4],
                            self.par_delay[1 + antenna * 8 + newch * 4],
                            self.par_delay[2 + antenna * 8 + newch * 4],
                            self.par_delay[3 + antenna * 8 + newch * 4],
                        ) = self.convert_time_offset_high(buff)[:]
