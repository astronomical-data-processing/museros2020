...
from astropy.coordinates import solar_system_ephemeris, EarthLocation, ITRS, AltAz
...
def sun_position(obs_time):
    ...
    # ITRF - Alta
    cAltAz = phasecentre.transform_to(AltAz(obstime=obs_time, location=location))
    newAltAzcoordiantes = SkyCoord(alt=cAltAz.alt, az=cAltAz.az, obstime=obs_time, frame='altaz')

    new_ra_dec = newAltAzcoordiantes.transform_to('icrs')
    print(new_ra_dec)

    c_ITRS = phasecentre.transform_to(ITRS(obstime=obs_time, location=location))
